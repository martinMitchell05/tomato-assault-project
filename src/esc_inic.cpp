...
esc_inic::esc_inic(Vector2u tam_resol, Time x_min) {
	//Configuración imagen que aparece al iniciar el juego
	...
	//Tipografía utilizada en el menu de inicio
	fuente.loadFromFile("DragonSlayer.ttf");
	//Configuración de la música del menú
	beat1.openFromFile("beat_1.ogg");
	beat1.play();
	beat1.setPlayingOffset(x_min);
	beat1.setLoop(true);
	beat1.setVolume(25.0);
	
	//Configuración de los botones jugar, puntajes, ayuda y salir 
	b_jugar.loadFromFile("jugar.png");
	...
	b_ptajes.loadFromFile("verpuntajes.png");
	...
	b_ayuda.loadFromFile("ayuda.png");
	...
	b_salir.loadFromFile("saliralesc.png");
...
void esc_inic :: actualizar(RenderWindow &w,juego &j) {
	...
	//Cuando el tiempo de la creación de pantalla haya sido mayor que 300 se podra comenzar a presionar los botones
	if (t_espera_inic.getElapsedTime().asMilliseconds() > 300 ){
		//Se obtiene el posicionamiento en pantalla de la imagen del botón
		FloatRect f1 = spr_jugar.getGlobalBounds();
		//Si el mouse se encuentra dentro de la imagen y si se presiona el click izquierdo en el, entonces la música se detiene y cambia de escena
