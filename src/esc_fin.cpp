...
esc_fin::esc_fin(string tiem_fin,Vector2u tam_resol): nom_jug(fuente,20,sf::Color::White) {
	...
	//Configuraci�n de la m�sica de la escena
	beat_2.openFromFile("sounds/beat_2.ogg");
	beat_2.play();
	beat_2.setVolume(25.0);
	beat_2.setLoop(true);
	
	//Se crea y configura los botones para volver a jugar y volver al men�
	...
	//Configuraci�n de textos que aparecen en la pantalla
	fin.setFont(fuente);
...
void esc_fin :: actualizar(RenderWindow &w,juego &j) {
	...
	if (t_espera_inic.getElapsedTime().asMilliseconds() > 300 ){
		...
		//Cuando se cumple la condici�n, se guarda el nombre del jugador en el archivo de puntuaciones
...
void esc_fin :: dibujar(RenderWindow &w, juego &j){
	...
	//Si ya guardo el nombre se dibuja el texto guardado con �xito
	if(!hay_que_cambiar){
		w.draw(guard_exi);
	}
