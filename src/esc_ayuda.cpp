...
esc_ayuda::esc_ayuda(Time x_min, Vector2u tam_resol) {
	//Configuración imagen que aparece al iniciar la escena de ayuda
	fondo.loadFromFile("fondoAyuda.png");
	...
	//Se configura el botón para volver al menú
...
void esc_ayuda::dibujar (RenderWindow & w, juego & j) {
	//Limpia la pantalla y dibuja la imagen de fondo y el botón
	w.clear(Color(0,0,0));
	w.draw(spr_fondo);
	w.draw(spr_volver);
}
